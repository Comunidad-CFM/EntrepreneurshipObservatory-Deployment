...
(function() {
	...
	function PersonasFactory($http, $q, API_URL) {
        ...
        function remove(id){
            ...
            $http({
                method: 'DELETE',
                url: API_URL + 'personas/destroy/' + id
            }).success(function(response){
                defered.resolve(response);
            })
            .error(function(err){
		...
		function getAll() {
			...
			$http.get(API_URL + 'personas/todas')
			.success(function(response) {
				defered.resolve(response);
			})
			.error(function(err) {
		...
		function getByType(type) {
			...
			$http.get(API_URL + 'personas/getByType/' + type)
			.success(function(response) {
				defered.resolve(response);
			})
			.error(function(err) {
        ...
        function getByTerritory(territory) {
            ...
            $http.get(API_URL + 'personas/getByTerritory/' + territory)
            .success(function(response) {
                defered.resolve(response);
            })
            .error(function(err) {
        ...
        function getBySector(sector) {
            ...
            $http.get(API_URL + 'personas/getBySector/' + sector)
            .success(function(response) {
                defered.resolve(response);
            })
            .error(function(err) {
		...
		function getPersona(idPersona) {
			...
			$http({
				method: 'POST',
				url: API_URL + 'personas/getPersona/',
				data: {id:idPersona}
			})
			.success(function(response) {
				defered.resolve(response);
        ...
        function isPass(id, currentPass) {
        	...
        	$http({
        		method: 'GET',
        		url: API_URL + 'personas/isPass/' + id + '/'+ currentPass
        	})
        	.success(function(response) {
        		defered.resolve(response[0]);
        	})
        ...
        function changePass(id, pass) {
        	...
        	$http({
        		method: 'POST',
        		url: API_URL + 'personas/changePass',
        		data: data
        	})
        	.success(function(response) {
        		defered.resolve(response);
